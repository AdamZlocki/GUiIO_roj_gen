...
def cso_algorithm(graph: GraphMatrix, vehicles: List[Vehicle], max_step: int, visual, matrix_d, vehicles_number: int = 3, inner_w: int = 1) -> None:
    """
    ...
    solutions = create_initial_solutions([], graph,vehicles, size_of_iteration=50)
    
    # posortowanie rozwiązań i zapamiętanie najlepszego
    solutions, pg = sort_solutions(solutions, size_of_iteration=50, best=0) 
    N = len(solutions)
    ...
    for _ in range(t_max):
        ...
        for sol_i in range(N):
            ...
            for sol_j in range(N):
                if cso_get_step_distance(solutions[sol_j], solutions[sol_i]) < visual and solutions[sol_j].time < solutions[sol_i].time:
            ...
            if pi is not None:
                ...
                pg = sort_solutions(solutions, size_of_iteration=50, best=pg)

            # 4 - Implementation of dispersion and updating pg
            solutions[sol_i] = dispersal(0.5, solutions[sol_i], solutions)
            pg = sort_solutions(solutions, size_of_iteration=50, best=pg)
            pg_list.append(pg)
